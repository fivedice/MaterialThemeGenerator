...
export class FontPickerComponent implements OnInit {
	...
	variants = ['regular', 'medium', 'bold', 'light'];
	...
	constructor(
		...
		this.searchItems = this.search.valueChanges.pipe(switchMap(x => this.searchAllFonts(x)));
	}

	searchAllFonts(q: string) {
		q = q.toLowerCase();
		return this.fontService.getAllFonts().pipe(map(x => x.items.filter(v => v.family.toLowerCase().startsWith(q))));
	}
