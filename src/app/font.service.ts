...
export class FontService {
	...
	public getAllFonts() {
		return this.http.get<{ items: FontMeta[] }>(`/assets/font-data.json`, {}).pipe(
			catchError(() => {
	...
	public loadFont(family: string) {
		...
		try {
			...
			link.onerror = x => console.error('Unable to load font:', family);
			link.href = `https://fonts.googleapis.com/css?family=${family.replace(/ /g, '+')}:400`;
			link.rel = 'stylesheet';

			document.head.appendChild(link);
		} catch (e) {
			console.error('Unable to load font:', family);
		}
	}
}
